...
void get_object(const obs_options *options, obs_object_info *object_info, obs_get_conditions *get_conditions,
                server_side_encryption_params *encryption_params, obs_get_object_handler *handler, void *callback_data)
{
    ...
    if (object_info->version_id) {
        safe_append("versionId", object_info->version_id, strlen(object_info->version_id),
    ...
    if (get_conditions && get_conditions->image_process_config) {
        ...
        if (isImageProcModeValid == 1) {
            ret = snprintf_s(strToAppend, sizeof(strToAppend), _TRUNCATE, "%s%s", strToAppend,
                             get_conditions->image_process_config->cmds_stylename);
            CheckAndLogNeg(ret, "snprintf_s", __FUNCTION__, __LINE__);

            safe_append("x-image-process", strToAppend, sizeof(strToAppend),
                        handler->response_handler.complete_callback);
        }
    }
    ...
    params.userProgressCallback = handler->progress_callback;
