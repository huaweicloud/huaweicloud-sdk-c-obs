                    ...
                    int namelen, int valuelen, char *c)
{
    ...
    if (!strncmp(header, "x-amz-request-id", namelen)||!strncmp(header, "x-obs-request-id", namelen)) {
    ...
    else_if (!strncmp(header, "x-amz-id-2", namelen)||!strncmp(header, "x-obs-id-2", namelen)) {
        responseProperties->request_id2 = 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Content-Type", namelen)) {
        responseProperties->content_type = 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Content-Length", namelen)) {
    ...
    else_if (!strncmp(header, "Server", namelen)) {
        responseProperties->server = 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "ETag", namelen)) {
        responseProperties->etag = 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-amz-expiration", namelen)||!strncmp(header, "x-obs-expiration", namelen)) {
        responseProperties->expiration= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if(!strncmp(header, "x-amz-website-redirect-location", namelen)||!strncmp(header, "x-obs-website-redirect-location", namelen)) {
        responseProperties->website_redirect_location= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "x-amz-version-id", namelen)||!strncmp(header, "x-obs-version-id", MAX(namelen, 16))) {
        responseProperties->version_id= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Access-Control-Allow-Origin", namelen)) {
        responseProperties->allow_origin= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Access-Control-Allow-Headers", namelen)) {
        responseProperties->allow_headers= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Access-Control-Max-Age", namelen)) {
        responseProperties->max_age= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Access-Control-Allow-Methods", namelen)) {
        responseProperties->allow_methods= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "Access-Control-Expose-Headers", namelen)) {
        responseProperties->expose_headers= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-default-storage-class", namelen)||!strncmp(header, "x-obs-storage-class", namelen)) {
        responseProperties->storage_class=
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-amz-storage-class", namelen)) {
        responseProperties->storage_class=
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-amz-server-side-encryption", namelen)||!strncmp(header, "x-obs-server-side-encryption", namelen)) {
    ...
    else_if (!strncmp(header, "x-amz-server-side-encryption-aws-kms-key-id", namelen)||!strncmp(header, "x-obs-server-side-encryption-aws-kms-key-id", namelen)) {
        responseProperties->kms_key_id= 
        string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
        valuelen, fit);
    }
    else_if (!strncmp(header, "x-amz-server-side-encryption-customer-algorithm", namelen)||!strncmp(header, "x-obs-server-side-encryption-customer-algorithm", namelen)) {
    ...
    else_if (!strncmp(header, "x-amz-server-side-encryption-customer-key-MD5", namelen)||!strncmp(header, "x-obs-server-side-encryption-customer-key-MD5", namelen)) {
        responseProperties->customer_key_md5= 
        string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
        valuelen, fit);
    }
    else_if (!strncmp(header, "x-obs-bucket-location", namelen)) {
        responseProperties->bucket_location=
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-obs-version", namelen)) {
        responseProperties->obs_version =
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-amz-restore", namelen)||!strncmp(header, "x-obs-restore", namelen)) {
        responseProperties->restore =
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-obs-object-type", namelen)) {
        responseProperties->obs_object_type =
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-obs-next-append-position", namelen)) {
        responseProperties->obs_next_append_position =
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-amz-epid", namelen)) {
        responseProperties->obs_head_epid  =
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, "x-obs-epid", namelen)) {
        responseProperties->obs_head_epid  =
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c,
                               valuelen, fit);
    }
    else_if (!strncmp(header, OBS_METADATA_HEADER_NAME_PREFIX, 
                      sizeof(OBS_METADATA_HEADER_NAME_PREFIX) - 1) ||
             !strncmp(header, "x-obs-meta-", sizeof("x-obs-meta-") - 1)) 
    {
